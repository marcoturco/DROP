...
brk_bias <- seq(0.1,2,0.2)
brk2_bias = union(-1e+05,brk_bias)
brk2_bias = union(brk2_bias,1e+05)
#col_bias <- (colorRampPalette(brewer.pal(9, "Oranges"))(8))

# Classic palette BuPu, with 4 colors
col_bias = brewer.pal(4, "PRGn") 
# I can add more tones to this palette :
col_bias = rev(colorRampPalette(col_bias)(11))
...
for (isc in 1:length(time_scale)) {
  ...
  for (idata in 1:length(datasets)) {
    ...
    postscript(
      file.path(
        dir_out,
        paste("MAE_MSWEP_", dataset, "_annual_spi", sc, ".eps",
              sep = "")
      ),
      paper = "special",
      width = 11,
    ...
    ce = 1.4
    ColorBarM(
      brks = brk2_bias, cols = col_bias,vert = T, cex = 1,labs = seq(2,length(brk2_bias) - 1,1)
    )
    ...
    save(corre, file = file.path(
      dir_out,
      paste("mae_spi_", sc, "_", dataset, "_MSWEP.RData", sep = "")
    ))
