...
#for (icaso in 1:1) {
 ...
 if (caso == "USA") {
  ...
  ilon = which(lon >= lon1-5 & lon <= lon1+5)
  ilat = which(lat >= lat1-5 & lat <= lat1+5)
  ...
  aux=scale(apply(obs[ilon, ilat , ], c(3), mean, na.rm = TRUE))
  # mswep=apply(obs[ilon, ilat , months], c(3), mean, na.rm = TRUE)
  mswep=aux[months]
  
  rm(aux)
  
  # aux=pred[ilon, ilat , months,1:10]*NA
  pred_data <- matrix(data = NA,
                  nrow = length(months),
                  ncol = 10)
  for (i in 1:10) {
    aux=scale(apply(pred[ilon, ilat ,, i], c(3), mean, na.rm = TRUE))
    pred_data[,i]=aux[months]
  ...
  aux=scale(apply(spi[ilon, ilat , ], c(3), mean, na.rm = TRUE))
  ens_mean=aux[months]
  ...
  ggsave(paste(dir_out,"/spi12_time_series", caso, "_regions.eps", sep = ""),height = 7 , width = 7 * aspect_ratio, device="eps")
