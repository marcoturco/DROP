...
dir_grace='/Users/marco/Documents/dati/GRACE'
...
for (isc in 1:length(time_scale)) {
  ...
  load(file.path(
    dir_grace,"GRACE_200204_201612.RData"
  ))
  grace=obs
  rm(obs)
...
#   
#   if (sc==12) {
#     grace = grace[, , seq(12, dim(grace)[3], 12)]
#   } else if (sc==6) {
#     grace = grace[, , seq(6, dim(grace)[3], 6)]
#   } else if (sc==3) {
#     grace = grace[, , seq(3, dim(grace)[3], 3)]
#   }
#   grace[is.infinite(grace)]=NA
  ...
  for (idata in 1:length(datasets)) {
    ...
    postscript(
      file.path(
        dir_out,
        paste("cor_GRACE_", dataset, "_annual_spi", sc, ".eps",
              sep = "")
      ),
      paper = "special",
      width = 11,
    ...
    save(corre, file = file.path(
      dir_out,
      paste("corre_spi_", sc, "_", dataset, "_GRACE.RData", sep = "")
    ))
